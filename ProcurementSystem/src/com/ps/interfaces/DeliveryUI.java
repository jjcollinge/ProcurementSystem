...
public class DeliveryUI extends UserInterface {
    ...
    private DeliveryUI() {
        ...
        confirmedItems = new ArrayList<Integer>();
    ...
    private void ascendBtn1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ascendBtn1ActionPerformed
        if(ascending1) {
            ...
            for(int i = 0; i < confirmedItems.size(); i++) {
                confirmedItems.set(i, checkBoxes.size() - confirmedItems.get(i) - 1);
            }
            System.out.println("reversed to : " + confirmedItems);
            updateCheckBoxes();
        }
    ...
    private void toggleCheckBoxes(int selectedIndex) {
        ...
        if(checkBoxes.get(selectedIndex).isVisible()) {
            ...
            if(confirmedItems.contains(selectedIndex))
                confirmedItems.remove(new Integer(selectedIndex));
        } else {
            // not checked so make it
            System.out.println("Showing");
            checkBoxes.get(selectedIndex).setVisible(true);
            confirmedItems.add(new Integer(selectedIndex));
        }      
        deliveryPanel.revalidate();
        deliveryPanel.repaint();
        System.out.println("updating: " + confirmedItems);
    }
    
    /**
     * Updates the check box graphics to match the model
     */
    public void updateCheckBoxes() {   
        // for all the checkboxes
        for(int i = 0; i < checkBoxes.size(); i++) {
            if(confirmedItems.contains(i)) {
                System.out.println("turning on index " + i);
                checkBoxes.get(i).setVisible(true);
            } else {
                System.out.println("turning off index " + i);
